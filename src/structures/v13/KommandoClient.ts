import { Client, Intents, Collection, ClientOptions, Interaction, Util as DJSUtil } from 'discord.js-13';
import { Util } from '.';
import fs from 'fs';
...
export class KommandoClient extends Client {
    ...
    constructor(options: IKommandoOptions, opts: ClientOptions = { intents: [ Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MEMBERS ]}) {
        super(<ClientOptions>DJSUtil.mergeDefault({ intents: [ Intents.FLAGS.GUILDS, Intents.FLAGS.GUILD_MEMBERS ]}, opts));
        
        if (!options || !has(options, 'directory')) throw new TypeError('Please provide the directory.');
        
        this.kommando = <typeof KommandoOptions>DJSUtil.mergeDefault(KommandoOptions, options);
